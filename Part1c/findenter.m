function [as, cs, s] = findenter(A, pie, c, indices)
...
tolerance = -1.0E-6;
% Obtain nonbasic indices
NBindices = setdiff(1:length(A), indices);
% Calculate nonbasic reduced costs
NBrc = c(NBindices)' - pie'*A(:, NBindices);

% Check optimality criteria
if all(NBrc > tolerance)
    ...
    s = NBindices(find(NBrc <= tolerance, 1));
    as = A(:, s);
    cs = c(s);
end 
end
